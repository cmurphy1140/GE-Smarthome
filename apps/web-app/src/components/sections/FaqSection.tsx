...
export default function FaqSection() {
  ...
  return (
    <section id="faqs" className="relative bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 py-24 text-white">
      {/* Animated background elements */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-900/20 via-transparent to-slate-900/30" />
      <div className="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-700 bg-gradient-to-br from-blue-800/10 via-blue-900/5 to-transparent" />

      {/* Floating orbs animation */}
      <div className="absolute top-4 right-4 h-24 w-24 rounded-full bg-gradient-to-r from-blue-400/20 to-blue-600/20 blur-xl opacity-50 hover:opacity-80 transition-all duration-1000 hover:scale-150" />
      <div className="absolute bottom-6 left-6 h-16 w-16 rounded-full bg-gradient-to-r from-blue-500/15 to-blue-700/15 blur-lg opacity-40 hover:opacity-70 transition-all duration-1000 hover:scale-125" />

      {/* Interactive glow effect */}
      <div className="absolute inset-0 rounded-3xl border border-blue-500/20 opacity-0 hover:opacity-100 transition-opacity duration-700" />
      <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-blue-500/5 via-transparent to-blue-600/5 opacity-0 hover:opacity-100 transition-opacity duration-700" />

      <div className="relative mx-auto grid max-w-6xl gap-16 px-4 sm:px-6 md:grid-cols-[1.1fr_0.9fr]">
            ...
            {faqs.map((faq, index) => {
              ...
              return (
                <motion.div
                  key={faq.question}
                  layout
                  className="group overflow-hidden rounded-3xl border border-blue-500/20 bg-black/20 backdrop-blur-sm text-white shadow-2xl transition-all duration-300 hover:shadow-[0_25px_60px_-12px_rgba(30,58,138,0.4)] hover:border-blue-400/30"
                >
                  <button
                    type="button"
                  ...
                  >
                    <div>
                      <p className="text-lg font-medium text-blue-300/70">
                    ...
                    <motion.div
                      ...
                      className="rounded-full border border-blue-400/30 bg-blue-500/20 backdrop-blur-sm p-2 text-blue-300 group-hover:bg-blue-400/30 group-hover:border-blue-300/50 group-hover:text-blue-200 transition-all duration-300"
                    >
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <motion.div
                        ...
                        className="px-8 pb-8 text-base leading-relaxed text-blue-200/80"
                      >
                        {faq.answer}
                      </motion.div>
                    )}
        ...
        <motion.div
          ...
          className="group relative overflow-hidden rounded-3xl border border-blue-500/20 bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-10 text-white shadow-2xl transition-all duration-300 hover:shadow-[0_25px_60px_-12px_rgba(30,58,138,0.4)] hover:border-blue-400/30"
        >
          {/* Animated background elements */}
          <div className="absolute inset-0 bg-gradient-to-br from-blue-900/20 via-transparent to-slate-900/30" />
